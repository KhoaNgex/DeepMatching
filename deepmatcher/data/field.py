...
class FastTextBinary(vocab.Vectors):
    ...
    def __init__(self, language="en", url_base=None, cache=None):
        ...
        base = (
            url_base
            or "https://dl.fbaipublicfiles.com/fasttext/vectors-wiki/wiki.{}.zip"
        )
    ...
    def cache(self, name, cache, url=None, backup_url=None):
        ...
        if not os.path.isfile(path) and url:
            if not os.path.exists(self.destination):
                logger.info(
                    "Downloading vectors from {} to {}".format(url, self.destination)
                )
                    ...
                    if "drive.google.com" in url:
                        try:
                            ...
                            if "quota was exceeded" in str(err):
                                logger.info(
                                    "Unable to fetch cached English Word Embeddings"
